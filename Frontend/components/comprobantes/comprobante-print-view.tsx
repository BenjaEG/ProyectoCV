      ...
      <div className="min-h-screen bg-zinc-100 print:bg-white">
        ...
        {isLoading ? (
          ...
          <div className="mx-auto max-w-4xl p-4 print:max-w-none print:p-0">
            <div className="rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm print:rounded-none print:border-0 print:shadow-none">
              <header className="border-b border-zinc-200 pb-6">
                <div className="flex items-start justify-between gap-6">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.35em] text-sky-700">Centro Vecinal</p>
                    <h2 className="mt-2 text-3xl font-bold tracking-tight text-zinc-900">Comprobante</h2>
                  ...
                  <div className="min-w-52 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-right">
                    ...
                    <p className="mt-2 text-2xl font-semibold text-zinc-900">{comprobante.numero}</p>
                    <p className="mt-3 text-sm text-zinc-600">Emitido: {comprobante.fechaEmision}</p>
                  </div>
                </div>
              </header>
              ...
              <section className="mt-8 grid gap-6 md:grid-cols-[1.4fr_1fr]">
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="mt-2 text-2xl font-semibold text-zinc-900">{comprobante.concepto}</p>
                  </div>
                  {comprobante.descripcion && (
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.25em] text-zinc-500">Descripción</p>
                      <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-zinc-700">{comprobante.descripcion}</p>
                    </div>
                  )}
                </div>
                <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-5">
                  ...
                  <dl className="mt-4 space-y-3 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3 border-t border-zinc-200 pt-3">
                      ...
                      <dd className="text-xl font-bold text-zinc-950">{formatCurrency(comprobante.monto)}</dd>
                    </div>
                  </dl>
                </div>
              </section>
              ...
              <footer className="mt-12 grid gap-8 border-t border-zinc-200 pt-8 md:grid-cols-2">
                <div>
                  ...
                  <p className="mt-3 text-sm text-zinc-600">Firma responsable</p>
                </div>
                <div>
                  <div className="h-16 border-b border-zinc-300" />
                  <p className="mt-3 text-sm text-zinc-600">Aclaración / recepción</p>
                </div>
              </footer>
            </div>
          </div>
